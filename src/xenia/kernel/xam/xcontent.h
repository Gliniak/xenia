...
struct XContentMetadata {
  ...
  vfs::VolumeDescriptor volume_descriptor;
  ...
  void set_device_id(const std::vector<uint8_t> new_device_id) {
    for (uint8_t i = 0; i < new_device_id.size(); i++) {
      device_id[i] = new_device_id[i];
    }
  }
