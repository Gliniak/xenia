...
dword_result_t XamUserCreateStatsEnumerator(
  ...
  if (buffer_size_ptr) {
    *buffer_size_ptr = static_cast<uint32_t>(entry_size) * stats_ptr->stats_amount;
  }

  auto e = object_ref<XStaticEnumerator>(
      new XStaticEnumerator(kernel_state(), count, flags));
