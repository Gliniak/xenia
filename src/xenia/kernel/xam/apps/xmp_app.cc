...
X_HRESULT XmpApp::XMPPlayTitlePlaylist(uint32_t playlist_handle,
  ...
  OnStateChanged();
  kernel_state_->BroadcastNotification(XN_XMP_PLAYBACKBEHAVIORCHANGED, 1);
  return X_E_SUCCESS;
}
...
void XmpApp::OnStateChanged() {
  kernel_state_->BroadcastNotification(XN_XMP_STATECHANGED,
                                       static_cast<uint32_t>(state_));
}
...
X_HRESULT XmpApp::DispatchMessageSync(uint32_t message, uint32_t buffer_ptr,
  ...
  switch (message) {
    ...
    case 0x00070008: {
      ...
      kernel_state_->BroadcastNotification(XN_XMP_PLAYBACKBEHAVIORCHANGED, 0);
      return X_E_SUCCESS;
    }
    case 0x00070009: {
      assert_true(!buffer_length || buffer_length == 8);
    ...
    case 0x0007001A: {
      ...
      kernel_state_->BroadcastNotification(XN_XMP_PLAYBACKCONTROLLERCHANGED,
