...
enum class XdbfSection : uint16_t {
  ...
  kTitle = 0x0004,
  kString = 0x0005,
  kProtectedAchievement = 0x0006
};

struct X_XDBF_ENTRY {
  xe::be<XdbfSection> section;
  xe::be<uint64_t> id;
...
struct X_XUSER_DATA {
  ...
  // 7 bytes padding
  union {
    xe::be<uint32_t> s32;      // X_XUSER_DATA_TYPE::kInt32
    xe::be<uint64_t> s64;      // X_XUSER_DATA_TYPE::kInt64
    xe::be<uint32_t> u32;
    xe::be<double> f64;  // X_XUSER_DATA_TYPE::kDouble
    struct                     // X_XUSER_DATA_TYPE::kUnicode
    {
      xe::be<uint32_t> size;  // Includes null-terminator
      xe::be<uint32_t> ptr;
    } unicode;
    xe::be<float> f32;  // X_XUSER_DATA_TYPE::kFloat
    struct {            // X_XUSER_DATA_TYPE::kBinary
      xe::be<uint32_t> size;
      xe::be<uint32_t> ptr;
    } binary;
    xe::be<uint64_t> filetime;  // X_XUSER_DATA_TYPE::kDateTime
  };
};
