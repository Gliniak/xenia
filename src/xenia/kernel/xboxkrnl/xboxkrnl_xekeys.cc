...
bool xeKeysLoadKeyVault() {
  ...
  auto kv_path = kernel_state()->emulator()->storage_root() / "KV.bin";
...
dword_result_t XeKeysLoadKeyVault_entry(lpvoid_t r3) {
  return xeKeysLoadKeyVault();
}
...
dword_result_t XeKeysQwNeRsaPrvCrypt_entry(dword_t key_idx,
                                           pointer_t<uint64_t> input,
...
dword_result_t XeKeysConsolePrivateKeySign_entry(
  ...
  if (!XeKeysQwNeRsaPrvCrypt_entry((uint32_t)XeKey::CONSOLE_PRIVATE_KEY,
                                   sig_buf, sig_buf)) {
    return false;
  }
...
dword_result_t XeKeysVerifyRSASignature_entry(dword_t use_live_key,
                                              lpvoid_t hash,
...
dword_result_t XeKeysAesCbcUsingKey_entry(lpvoid_t obscured_key,
  ...
  XeCryptAesKey_entry(&aes,
                      xeKeysGetKeyPtr<uint8_t>(XeKey::KEY_OBFUSCATION_KEY));
...
dword_result_t XeKeysObfuscate_entry(dword_t roaming, lpvoid_t input,
  ...
  *output_size = input_size + 0x18;
  ...
  auto result = XeKeysHmacSha_entry((uint32_t)key_idx, output_ptr + 0x10,
                                    *output_size - 0x10, nullptr, 0, nullptr, 0,
                                    output_ptr, 0x10);
  if (XFAILED(result)) {
    return result;
  }
  ...
  uint8_t key[0x10];
  auto result2 = XeKeysHmacSha_entry((uint32_t)key_idx, output_ptr, 0x10, 0, 0,
                                     0, 0, key, 0x10);
...
dword_result_t XeKeysUnObfuscate_entry(dword_t roaming, lpvoid_t input,
  ...
  *output_size = input_size - 0x18;
  ...
  uint8_t key[0x10];
  auto result = XeKeysHmacSha_entry(key_idx, hmac_header, 0x10, nullptr, 0,
                                    nullptr, 0, key, 0x10);
  if (XFAILED(result)) {
    return false;
  }
...
DECLARE_XBOXKRNL_EMPTY_REGISTER_EXPORTS(XKeys);