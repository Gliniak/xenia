...
SvodContainerFile::SvodContainerFile(SvodContainerEntry* entry)
    : XContentContainerFile(0, entry), entry_(entry) {}

SvodContainerFile::~SvodContainerFile() = default;

void SvodContainerFile::Destroy() {
  // Do not flush file if device is for whatever reason closed
  //auto device = reinterpret_cast<StfsContainerDevice*>(entry_->device());
  delete this;
}

X_STATUS SvodContainerFile::ReadSync(void* buffer, size_t buffer_length,
    size_t byte_offset,
